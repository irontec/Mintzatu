...
class tablonfeednews extends tablon {
	var $feedUrl;
	var $feedId;
	...
	function __construct(&$conf) {
		...
		$this->aJs[] = "../modules/tablonfeednews/scripts/savenews.js";
	...
	public function loadFeed($pl){
		...
		$feedId = $this->conf[$this->currentSection]['idcond'];


		$condiciones = $this->buildConds();
		$condiciones = $condiciones[0];
		$sql = " select {$feedId} as feedid, {$feed} as feedurl from ".$pl->getTab() ;
		$sql = $sql." ".$condiciones;
		$c = new con($sql);
		$r = $c->getResult();
		$this->feedUrl = $r['feedurl'];
		$this->feedId = $r['feedid'];
	}
	...
	public function drawFeeds(){
		...
		if ($rs = $rss->get($url_flux_rss)){
			...
			for($i=0;$i<$l;$i++){
				...
				$id = i::clean(trim(html_entity_decode($this->swencoding($this->feedId."_".$rs['items'][$i]['pubDate']."_".$rs['items'][$i]['title'],$rss->encoding))));
				echo '<tr>';
				echo '<td class="vals_v_'.$id.'" id="pubdate_v_'.$id.'">'.$rs['items'][$i]['pubDate'].'</td>';
				echo '<td class="vals_v_'.$id.'" id="title_v_'.$id.'"> ';
				echo '<a  class="nomove vals_v_'.$id.'" id="link_v_'.$i.'" href="'.$rs['items'][$i]['link'].'" >';
				echo html_entity_decode($this->swencoding($rs['items'][$i]['title'],$rss->encoding));
				echo '</a></td>';
				...
				if (!isset($this->conf[$this->currentSection]['bot']) || trim($this->conf[$this->currentSection]['bot']) == ""){
					$this->conf[$this->currentSection]['bot'] = "hdd_mount.png";
				}

				$href = "";
				if(isset($this->conf[$this->currentSection]["feedmapplt"])){
					$href =  $this->conf[$this->currentSection]["feedmapplt"];
				}else if(isset($this->conf[$this->currentSection]['cFich'])){
					$href = base64_encode($this->conf[$this->currentSection]['cFich']);
				}

				echo '<td id="v_'.$id.'">
				<a href="'.$href.'"  class="feedButtom opts" ref="'.$this->currentValue.'" title="'.$this->conf[$this->currentSection]['bottitle'].'" >
