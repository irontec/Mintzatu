<?php
class tablonfullcalendar_defcolors extends tablon {
	...
	protected $noShowFieldsOnlyFade = false;
	...
	protected function drawDayContents($arr) {
		...
		foreach ($arr as $r) {
			...
			echo 'style="display:block;background:'.$r['colorDefinitionDecisive'].';color:#000000;float:left;"';
	...
	public function getColors(){
		...
		//$theColor = $this->plantillaPrincipal->aFields['colorDefinitionDecisive'];
		if ($cd<$cd2){
			$x = 2;
		...
		//$back = $this->plantillaPrincipal->fields['colorDefinitionDecisive']->getValue();
		$back = $this->getHex($R1).$this->getHex($G1).$this->getHex($B1);
		return array($back,"#000000");
	...
	public function drawCalendarD($t){
		...
		foreach ($this->aDateResults as $id=>$aInfo){
			...
			$this->aDateResults[$id]['color'] = $this->aDateResults[$id]['colorDefinitionDecisive'];
			//$this->aDateResults[$id]['fontcolor'] = $colors[1];
			$this->aDateResults[$id]['fontcolor'] = "#000000";
...
//			echo $aInfo['__ID'].' => '.	$falsestart.' =>'.$falseend.' '.$aInfo['FC_startField'].' '.$aInfo['FC_endField'].' * '.	date('r',$falsestart).' =>'.date('r',$falseend);
			...
			for ($m=0;$m<60;$m=$m+$intTime){
				...
				if (isset($aDraw[$time])){
					foreach ($aDraw[$time] as $event){
						...
						echo '<div style="display:block;background:'.$this->aDateResults[$event]['color'].';color:'.$this->aDateResults[$event]['fontcolor'].';float:left;border:.25px solid white;" ';
		...
		$aNoMostar = array_merge($this->getNoShownFields(),$this->getNoShownFieldsOnlyFade());
		echo $this->plantillaPrincipal->drawHead(($this->hasOptions()&&($opts = $this->drawOptions("%id%"))),$aNoMostar,$multiops,krm_menu::getURL().$this->getHistoryURL());
	...
	public function loadNoShownFieldsOnlyFade() {
		$this->noShowFieldsOnlyFade = array("colorDefinitionDecisive");
		//colorDefinitionDecisive
		if (isset($this->selectedConf['noshowOnFade'])||isset($this->selectedConf['showOnFade'])) {
			if (isset($this->selectedConf['noshowOnFade'])){
				$tmpnoshow = explode(",",$this->selectedConf['noshowOnFade']);
					...
					if (preg_match("/\*/",$no)) { // si tiene asteriskos ...
						...
						foreach ($this->plantillaPrincipal->aFields as $field=>$foo){
							if (preg_match("/".$exp."/",$field)) {
								$this->noShowFieldsOnlyFade[] = $field;
							}
						}
					}
					$this->noShowFieldsOnlyFade[] = $no;
				}
			}
			if (isset($this->selectedConf['showOnFade'])){
				$this->noShowFieldsOnlyFade = array();
				$aShow = explode(",",$this->selectedConf['showOnFade']);
				foreach ($this->plantillaPrincipal->aFields as $field=>$foo){
					if (!in_array($field,$aShow))	$this->noShowFieldsOnlyFade[] = $field;
				}
			}
		}
	}

	public function getNoShownFieldsOnlyFade() {
		if(($this->noShowFieldsOnlyFade==false||!is_array($this->noShowFieldsOnlyFade))){$this->loadNoShownFieldsOnlyFade();}
		return $this->noShowFieldsOnlyFade;
	}

	public function isNoShownOnlyFade($idx) {
		return (in_array($idx,$this->getNoShownFieldsOnlyFade()));
	}
	...
	protected function drawTableContents($checkBox = false,$csv = false) {
		...
		foreach ($this->aDateResults as $r) {
			...
			for ($i=0;$i<$pl->getNumFields();$i++) {
				...
				if ($this->isNoShownOnlyFade($pl->fields[$i]->getIndex())){
					continue;
				}
