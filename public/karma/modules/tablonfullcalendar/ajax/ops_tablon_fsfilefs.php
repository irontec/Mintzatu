<?php
	...
	switch($_GET['acc']) {
		case "list":
			if (($campos = tablon_AJAXjeditable::decodeId($_GET['id']))=== false) die("Error indefinido");
			...
			if (!$pl->fields[$idFLD]->isSourceFileSystem()) die("Error accediendo al campo");
			...
			if( file_exists($real_path) ) {
                ...
                foreach( $files as $file ) {
                        if( file_exists($real_path . $file) && $file != '.' && $file != '..' && !is_dir($real_path . $file) ) {
                                ...
                                echo "<li class=\"file ext_$ext\"><a href=\"#\" rel=\"" . htmlentities($dir . $file) . ";" . basename($file). ";". filesize($real_path.$file) . ";". i::mime_content_type($real_path.$file) ."\">" . htmlentities($file) . "</a></li>";
                        }
                }
                echo "</ul>";
        }
