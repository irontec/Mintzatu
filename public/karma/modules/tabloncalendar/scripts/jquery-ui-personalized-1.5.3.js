...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		onSelectUpdate: true, // True to update datepicker after selecting a date, false to do nothing.
...
$.extend(Datepicker.prototype, {
	...
	_doTheDay: function(id,month,year,day){
		var target = $(id);
		var inst = $.data(target[0], PROP_NAME);
		//if(day == '1' && month == '2' && year == '2009') alert(month+"-"+year+"-"+day);
		return this._formatDate(inst, day, month, year);
	...
	_selectDate: function(id, dateStr) {
		...
		if (inst.inline)
			if(inst.settings['onSelectUpdate'] == true)	this._updateDatepicker(inst);
			...
			//for (var col = 0; col < 4; col++) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						var elDate = printDate.getDate();
						var elIDtd = (unselectable?'':' id = "tdDaysCell_'+this._doTheDay('#'+inst.id,drawMonth, drawYear, elDate)+'"');
						var elIDa = (unselectable?'':' id = "'+this._doTheDay('#'+inst.id,drawMonth, drawYear, elDate)+'"');
						html += '<td class="ui-datepicker-days-cell' +
							...
							inst.id + '\',' + drawMonth + ',' + drawYear + ', this);"') +elIDtd+'>' + // actions
							(otherMonth ? (showOtherMonths ? printDate.getDate() : '&#xa0;') : // display for other months
							(unselectable ? printDate.getDate() : '<a'+elIDa+'>' + printDate.getDate() + '</a>')) + '</td>'; // display for this month
