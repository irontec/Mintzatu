...
class tablon_FLDenumldap extends tablon_FLDenum {
	public $type="select";
	private $keys = array();
	...
	public function getSQLType() {
		return "varchar (150)" . $this->getSQLRequired() . $this->getSQLDefaultValue();
	}
	...
	private function loadKeysValues($a=false) {
		...
		$ds=ldap_connect($this->conf['server']);

		$r=ldap_bind($ds);

		$idAlias = (isset($this->conf['id_alias']))? $this->conf['id_alias']:"uid";
		$valueAlias = (isset($this->conf['value_alias']))? $this->conf['value_alias']:"cn";

		$attributes = array($idAlias,$valueAlias);

		$sr=ldap_search($ds,$this->conf['cad'],$this->conf['filtro'],$attributes);

		$info = ldap_get_entries($ds, $sr);

		for ($i=0; $i<$info["count"]; $i++) {
			if (isset($info[$i][$idAlias]))
			{
				$this->keys[] = $info[$i][$idAlias][0];
				$this->values[] = $info[$i][$valueAlias][0];
			}
		}
