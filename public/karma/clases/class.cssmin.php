...
class cssmin
	...
	public static function minify($css, $dirname, $options = "remove-last-semicolon")
		{
		...
		if (in_array("iron-base64-images", $options))
		{
			// Encode url() to base64
			$css = preg_replace_callback("/url\s*\((.*)\)/siU", "cssmin_base64_images", $css);
		}	
		// Remove comments
 ...
 * @return	string
 */
function cssmin_base64_images($match) {
	$match[1] = str_replace("'","",$match[1]);
	if (file_exists(cssmin::$dirname. '/' . $match[1])) {
		return "url(data:image/png;base64," . base64_encode(file_get_contents(cssmin::$dirname. '/' . $match[1])) . ")";
	}
	return "url(" . $match[1] . ")";
