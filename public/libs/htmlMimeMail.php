...
*/
	...
	function _encodeHeader2($input, $charset = 'ISO-8859-1') {
   if( ereg("[^A-Za-z0-9\ ]", $input) ) {
       $text = "=?".$charset."?Q?";
       for( $i = 0 ; $i < strlen($input) ; $i++ ) {
           if( ereg("[^A-Za-z0-9]", $input[$i]) ) {
               $text .= "=".dechex(ord($input[$i]));
           } else {
               $text .= $input[$i];
           }
       }
		...
		switch ($type) {
			case 'mail':
				...
				if (!empty($this->headers['Subject'])) {
					$subject = $this->_encodeHeader2($this->headers['Subject'], $this->build_params['head_charset']);
