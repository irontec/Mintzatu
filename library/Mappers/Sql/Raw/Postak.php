...
/**
 * Data Mapper implementation for Mintzatu_Model_Postak
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Postak extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Postak $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Postak) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'id_posta' => $model->getIdPosta(),
            'identifikatzailea' => $model->getIdentifikatzailea(),
            'mezua_html' => $model->getMezuaHtml(),
            'mezua_text' => $model->getMezuaText(),
        );
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Postak
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Postak');
        }
     ...
     *
     * @param Mintzatu_Model_Postak $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Postak', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_posta = ?', $model->getIdPosta());
     ...
     */
    public function save(\Mintzatu_Model_Postak $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdPosta();
        $success = true;
     ...
     * @param Mintzatu_Model_Postak|null $model
     * @return Mintzatu_Model_Postak|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Postak_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Postak_".$primary_key);
     ...
     * @param Mintzatu_Model_Postak|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Postak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Postak();
        }

        if (is_array($data)) {
            $entry->setIdPosta($data['id_posta'])
                  ->setIdentifikatzailea($data['identifikatzailea'])
                  ->setMezuaHtml($data['mezua_html'])
                  ->setMezuaText($data['mezua_text']);
