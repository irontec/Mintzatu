...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaUsuarios
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaUsuarios extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaUsuarios $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaUsuarios) {
        ...
        $result = array(
            ...
            'fecha_nacimiento' => $model->getFechaNacimiento(),
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaUsuarios
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaUsuarios');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaUsuarios $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaUsuarios', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_usuario = ?', $model->getIdUsuario());
     ...
     */
    public function save(\Mintzatu_Model_KarmaUsuarios $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdUsuario();
        $success = true;
        ...
        try {
            if ($primary_key === null or empty($primary_key)) {
                ...
                if ($primary_key) {
                    $model->setIdUsuario($primary_key);
                } else {
            ...
            if ($recursive) {
                if ($model->getKarmaRelUsuariosRoles(false) !== null) {
                    ...
                    foreach ($KarmaRelUsuariosRoles as $value) {
                        $value->setIdUsuario($primary_key)
     ...
     * @param Mintzatu_Model_KarmaUsuarios|null $model
     * @return Mintzatu_Model_KarmaUsuarios|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaUsuarios_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaUsuarios_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaUsuarios|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaUsuarios The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaUsuarios();
        }

        if (is_array($data)) {
            $entry->setIdUsuario($data['id_usuario'])
                  ->setLogin($data['login'])
                  ->setPass($data['pass'])
