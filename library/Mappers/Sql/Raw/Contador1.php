...
/**
 * Data Mapper implementation for Mintzatu_Model_Contador1
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Contador1 extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Contador1 $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Contador1) {
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Contador1
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Contador1');
        }
     ...
     *
     * @param Mintzatu_Model_Contador1 $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Contador1', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto(' = ?', $model->get());
     ...
     */
    public function save(\Mintzatu_Model_Contador1 $model,
        ...
        $mainPrimaryKey = $primary_key = $model->get();
        $success = true;
     ...
     *
     * @param  $primary_key
     * @param Mintzatu_Model_Contador1|null $model
     * @return Mintzatu_Model_Contador1|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Contador1_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Contador1_".$primary_key);
     ...
     * @param Mintzatu_Model_Contador1|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Contador1 The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Contador1();
        }
