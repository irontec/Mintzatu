...
/**
 * Data Mapper implementation for Mintzatu_Model_LekuenIrudiak
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class LekuenIrudiak extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_LekuenIrudiak $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_LekuenIrudiak) {
        ...
        $result = array(
            ...
            'id_erabiltzailea' => $model->getIdErabiltzailea(),
     ...
     *
     * @return Mappers\\Sql\\DbTable\\LekuenIrudiak
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\LekuenIrudiak');
        }
     ...
     *
     * @param Mintzatu_Model_LekuenIrudiak $model The model to delete
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\LekuenIrudiak', $capitzalizedFk);
     ...
     */
    public function save(\Mintzatu_Model_LekuenIrudiak $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdIrudia();
        $success = true;
     ...
     * @param Mintzatu_Model_LekuenIrudiak|null $model
     * @return Mintzatu_Model_LekuenIrudiak|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_LekuenIrudiak_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_LekuenIrudiak_".$primary_key);
     ...
     * @param Mintzatu_Model_LekuenIrudiak|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_LekuenIrudiak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_LekuenIrudiak();
        }

        if (is_array($data)) {
            $entry->setIdIrudia($data['id_irudia'])
                  ->setIdLekua($data['id_lekua'])
                  ->setIdErabiltzailea($data['id_erabiltzailea'])
