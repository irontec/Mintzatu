...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaRoles
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaRoles extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaRoles $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaRoles) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'id_rol' => $model->getIdRol(),
            'rol' => $model->getRol(),
            'descripcion' => $model->getDescripcion(),
            'borrado' => $model->getBorrado(),
            'fecha_insert' => $model->getFechaInsert(),
        );
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaRoles
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaRoles');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaRoles $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaRoles', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_rol = ?', $model->getIdRol());
     ...
     */
    public function save(\Mintzatu_Model_KarmaRoles $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdRol();
        $success = true;
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getKarmaRelRolesMenus(false) !== null) {
                    $KarmaRelRolesMenus = $model->getKarmaRelRolesMenus();

                    if (!is_array($KarmaRelRolesMenus)) {
                        
                        $KarmaRelRolesMenus = array($KarmaRelRolesMenus);
                    }
                    
                    foreach ($KarmaRelRolesMenus as $value) {
                        $value->setIdRol($primary_key)
                              ->save($ignoreEmptyValues, $recursive, false, $transactionTag);
                    }
                }

                if ($model->getKarmaRelUsuariosRoles(false) !== null) {
                    $KarmaRelUsuariosRoles = $model->getKarmaRelUsuariosRoles();

                    if (!is_array($KarmaRelUsuariosRoles)) {
                        
                        $KarmaRelUsuariosRoles = array($KarmaRelUsuariosRoles);
                    }
                    
                    foreach ($KarmaRelUsuariosRoles as $value) {
                        $value->setIdRol($primary_key)
                              ->save($ignoreEmptyValues, $recursive, false, $transactionTag);
                    }
                }
     ...
     * @param Mintzatu_Model_KarmaRoles|null $model
     * @return Mintzatu_Model_KarmaRoles|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaRoles_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaRoles_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaRoles|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaRoles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaRoles();
        }

        if (is_array($data)) {
            $entry->setIdRol($data['id_rol'])
                  ->setRol($data['rol'])
                  ->setDescripcion($data['descripcion'])
                  ->setBorrado($data['borrado'])
                  ->setFechaInsert($data['fecha_insert']);
