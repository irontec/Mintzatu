...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaImgCategorias
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaImgCategorias extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaImgCategorias $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaImgCategorias) {
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaImgCategorias
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaImgCategorias');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaImgCategorias $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaImgCategorias', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('idCategoria = ?', $model->getIdCategoria());
     ...
     */
    public function save(\Mintzatu_Model_KarmaImgCategorias $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdCategoria();
        $success = true;
        ...
        try {
            if ($primary_key === null or empty($primary_key)) {
                ...
                if ($primary_key) {
                    $model->setIdCategoria($primary_key);
                } else {
     ...
     * @param Mintzatu_Model_KarmaImgCategorias|null $model
     * @return Mintzatu_Model_KarmaImgCategorias|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaImgCategorias_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaImgCategorias_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaImgCategorias|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaImgCategorias The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaImgCategorias();
        }

        if (is_array($data)) {
            $entry->setIdCategoria($data['idCategoria'])
