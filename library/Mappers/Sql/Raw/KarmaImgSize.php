...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaImgSize
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaImgSize extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaImgSize $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaImgSize) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'idSize' => $model->getIdSize(),
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaImgSize
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaImgSize');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaImgSize $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaImgSize', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('idSize = ?', $model->getIdSize());
     ...
     */
    public function save(\Mintzatu_Model_KarmaImgSize $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdSize();
        $success = true;
     ...
     * @param Mintzatu_Model_KarmaImgSize|null $model
     * @return Mintzatu_Model_KarmaImgSize|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaImgSize_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaImgSize_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaImgSize|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaImgSize The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaImgSize();
        }

        if (is_array($data)) {
            $entry->setIdSize($data['idSize'])
