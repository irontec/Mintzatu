...
/**
 * Data Mapper implementation for Mintzatu_Model_Orrialdeak
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Orrialdeak extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Orrialdeak $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Orrialdeak) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'id_orrialde' => $model->getIdOrrialde(),
            'identifikatzailea' => $model->getIdentifikatzailea(),
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Orrialdeak
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Orrialdeak');
        }
     ...
     *
     * @param Mintzatu_Model_Orrialdeak $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Orrialdeak', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_orrialde = ?', $model->getIdOrrialde());
     ...
     */
    public function save(\Mintzatu_Model_Orrialdeak $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdOrrialde();
        $success = true;
     ...
     * @param Mintzatu_Model_Orrialdeak|null $model
     * @return Mintzatu_Model_Orrialdeak|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Orrialdeak_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Orrialdeak_".$primary_key);
     ...
     * @param Mintzatu_Model_Orrialdeak|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Orrialdeak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Orrialdeak();
        }

        if (is_array($data)) {
            $entry->setIdOrrialde($data['id_orrialde'])
                  ->setIdentifikatzailea($data['identifikatzailea'])
                  ->setTitulua($data['titulua'])
                  ->setEdukia($data['edukia'])
                  ->setNoiz($data['noiz']);
