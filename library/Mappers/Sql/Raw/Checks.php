...
/**
 * Data Mapper implementation for Mintzatu_Model_Checks
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Checks extends MapperAbstract
{
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Checks
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Checks');
        }
     ...
     *
     * @param Mintzatu_Model_Checks $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Checks', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_check = ?', $model->getIdCheck());
     ...
     */
    public function save(\Mintzatu_Model_Checks $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdCheck();
        $success = true;
     ...
     * @param Mintzatu_Model_Checks|null $model
     * @return Mintzatu_Model_Checks|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Checks_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Checks_".$primary_key);
     ...
     * @param Mintzatu_Model_Checks|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Checks The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Checks();
        }

        if (is_array($data)) {
            $entry->setIdCheck($data['id_check'])
                  ->setIdErabiltzaile($data['id_erabiltzaile'])
                  ->setIdLekua($data['id_lekua'])
                  ->setIruzkina($data['iruzkina'])
                  ->setNoiz($data['noiz'])
                  ->setNondik($data['nondik']);
