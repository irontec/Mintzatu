...
/**
 * Data Mapper implementation for Mintzatu_Model_Aktibitatea
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Aktibitatea extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Aktibitatea $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Aktibitatea) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'id_aktibitatea' => $model->getIdAktibitatea(),
            'id_lerroa' => $model->getIdLerroa(),
            'taula' => $model->getTaula(),
            'akzioa' => $model->getAkzioa(),
            'noiz' => $model->getNoiz(),
        );
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Aktibitatea
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Aktibitatea');
        }
     ...
     *
     * @param Mintzatu_Model_Aktibitatea $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Aktibitatea', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_aktibitatea = ?', $model->getIdAktibitatea());
     ...
     */
    public function save(\Mintzatu_Model_Aktibitatea $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdAktibitatea();
        $success = true;
     ...
     * @param Mintzatu_Model_Aktibitatea|null $model
     * @return Mintzatu_Model_Aktibitatea|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Aktibitatea_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Aktibitatea_".$primary_key);
     ...
     * @param Mintzatu_Model_Aktibitatea|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Aktibitatea The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Aktibitatea();
        }

        if (is_array($data)) {
            $entry->setIdAktibitatea($data['id_aktibitatea'])
                  ->setIdLerroa($data['id_lerroa'])
                  ->setTaula($data['taula'])
                  ->setAkzioa($data['akzioa'])
                  ->setNoiz($data['noiz']);
