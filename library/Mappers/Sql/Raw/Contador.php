...
/**
 * Data Mapper implementation for Mintzatu_Model_Contador
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Contador extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Contador $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Contador) {
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Contador
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Contador');
        }
     ...
     *
     * @param Mintzatu_Model_Contador $model The model to delete
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Contador', $capitzalizedFk);
     ...
     */
    public function save(\Mintzatu_Model_Contador $model,
     ...
     *
     * @param  $primary_key
     * @param Mintzatu_Model_Contador|null $model
     * @return Mintzatu_Model_Contador|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Contador_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Contador_".$primary_key);
     ...
     * @param Mintzatu_Model_Contador|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Contador The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Contador();
        }
