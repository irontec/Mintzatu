...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaRelRolesMenus
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaRelRolesMenus extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaRelRolesMenus $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaRelRolesMenus) {
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaRelRolesMenus
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaRelRolesMenus');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaRelRolesMenus $model The model to delete
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaRelRolesMenus', $capitzalizedFk);
     ...
     */
    public function save(\Mintzatu_Model_KarmaRelRolesMenus $model,
     ...
     * @param Mintzatu_Model_KarmaRelRolesMenus|null $model
     * @return Mintzatu_Model_KarmaRelRolesMenus|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaRelRolesMenus_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaRelRolesMenus_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaRelRolesMenus|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaRelRolesMenus The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaRelRolesMenus();
        }

        if (is_array($data)) {
            $entry->setIdRel($data['id_rel'])
                  ->setIdRol($data['id_rol'])
                  ->setKeyMenu($data['key_menu'])
