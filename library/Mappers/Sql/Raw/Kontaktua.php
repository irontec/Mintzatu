...
/**
 * Data Mapper implementation for Mintzatu_Model_Kontaktua
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Kontaktua extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Kontaktua $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Kontaktua) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'id_kontaktua' => $model->getIdKontaktua(),
            'izena' => $model->getIzena(),
            'posta' => $model->getPosta(),
            'mezua' => $model->getMezua(),
            'noiz' => $model->getNoiz(),
        );
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Kontaktua
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Kontaktua');
        }
     ...
     *
     * @param Mintzatu_Model_Kontaktua $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Kontaktua', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_kontaktua = ?', $model->getIdKontaktua());
     ...
     */
    public function save(\Mintzatu_Model_Kontaktua $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdKontaktua();
        $success = true;
     ...
     * @param Mintzatu_Model_Kontaktua|null $model
     * @return Mintzatu_Model_Kontaktua|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Kontaktua_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Kontaktua_".$primary_key);
     ...
     * @param Mintzatu_Model_Kontaktua|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Kontaktua The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Kontaktua();
        }

        if (is_array($data)) {
            $entry->setIdKontaktua($data['id_kontaktua'])
                  ->setIzena($data['izena'])
                  ->setPosta($data['posta'])
                  ->setMezua($data['mezua'])
                  ->setNoiz($data['noiz']);
