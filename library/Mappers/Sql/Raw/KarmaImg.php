...
/**
 * Data Mapper implementation for Mintzatu_Model_KarmaImg
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class KarmaImg extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_KarmaImg $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_KarmaImg) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'idImg' => $model->getIdImg(),
            'idCategoria' => $model->getIdCategoria(),
            'Titulo' => $model->getTitulo(),
            'img_binario' => $model->getImgBinario(),
            'nombre_img' => $model->getNombreImg(),
            'size_img' => $model->getSizeImg(),
        );
     ...
     *
     * @return Mappers\\Sql\\DbTable\\KarmaImg
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\KarmaImg');
        }
     ...
     *
     * @param Mintzatu_Model_KarmaImg $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\KarmaImg', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('idImg = ?', $model->getIdImg());
     ...
     */
    public function save(\Mintzatu_Model_KarmaImg $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdImg();
        $success = true;
     ...
     * @param Mintzatu_Model_KarmaImg|null $model
     * @return Mintzatu_Model_KarmaImg|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_KarmaImg_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_KarmaImg_".$primary_key);
     ...
     * @param Mintzatu_Model_KarmaImg|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_KarmaImg The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_KarmaImg();
        }

        if (is_array($data)) {
            $entry->setIdImg($data['idImg'])
                  ->setIdCategoria($data['idCategoria'])
                  ->setTitulo($data['Titulo'])
                  ->setImgBinario($data['img_binario'])
                  ->setNombreImg($data['nombre_img'])
                  ->setSizeImg($data['size_img']);
