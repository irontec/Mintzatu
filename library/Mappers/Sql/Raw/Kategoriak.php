...
/**
 * Data Mapper implementation for Mintzatu_Model_Kategoriak
 *
 * @package Mappers\Sql
 * @subpackage Raw
 * @author <Lander Ontoria Gardeazabal>
...
class Kategoriak extends MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Mintzatu_Model_Kategoriak $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof \Mintzatu_Model_Kategoriak) {
     ...
     *
     * @return Mappers\\Sql\\DbTable\\Kategoriak
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Mappers\\Sql\\DbTable\\Kategoriak');
        }
     ...
     *
     * @param Mintzatu_Model_Kategoriak $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (! isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) and class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Mappers\\Sql\\DbTable\\Kategoriak', $capitzalizedFk);
            ...
            $where = $this->getDbTable()->getAdapter()->quoteInto('id_kategoria = ?', $model->getIdKategoria());
     ...
     */
    public function save(\Mintzatu_Model_Kategoriak $model,
        ...
        $mainPrimaryKey = $primary_key = $model->getIdKategoria();
        $success = true;
            ...
            if ($recursive) {
                if ($model->getLekuak(false) !== null) {
                    ...
                    foreach ($Lekuak as $value) {
                        $value->setIdKategoria($primary_key)
     ...
     * @param Mintzatu_Model_Kategoriak|null $model
     * @return Mintzatu_Model_Kategoriak|null The object provided or null if not found
     */
    public function find($primary_key, $model = null)
    {
        if (!($this->_cache and $this->_cache->test("Mintzatu_Model_Kategoriak_".$primary_key))) {
            ...
            $tmp = $this->_cache->load("Mintzatu_Model_Kategoriak_".$primary_key);
     ...
     * @param Mintzatu_Model_Kategoriak|null $entry The object to load the data into, or null to have one created
     * @return Mintzatu_Model_Kategoriak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Mintzatu_Model_Kategoriak();
        }

        if (is_array($data)) {
            $entry->setIdKategoria($data['id_kategoria'])
                  ->setIzena($data['izena'])
